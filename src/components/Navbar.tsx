    ...
    return (
        <nav className='flex items-center justify-between p-6 container mx-auto bg-white w-full'>
            <div className="logo flex items-center gap-2">
                ...
                <h3 className='font-bold text-xl text-gray-900'>TradeJourney</h3>
            </div>

            <div className="links flex items-center gap-12">
                {navLinks.map((link, idx) => (
                    <Link className="text-gray-700 font-medium hover:text-primary-blue" key={idx} href={link.route}>{link.name}</Link>
                ))}
